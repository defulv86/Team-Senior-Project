...
function loadNotifications(){
    const notificationList = document.getElementById('notification-body');
    ...
    .then(data => {
        if (data.length === 0) {
            ...
            data.forEach(notif => {
                ...
                if (hoursDiff < 24) {
                    dateSpan.textContent = `${new Date(notif.time_created).toLocaleString([], { hour: '2-digit', minute: '2-digit' })}`;
                }
                else{
