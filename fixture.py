...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'ppst.settings')  # Replace 'ppst.settings' with your actual settings module
django.setup()
...
# Fake data for age groups 40-49 and 50-59
age_groups = {
    (40, 49): 3,  # 3 tests for age group 40-49
    (50, 59): 1   # 1 test for age group 50-59
}

for (min_age, max_age), test_count in age_groups.items():
    for i in range(test_count):
        ...
        test = Test.objects.create(
            ...
            finished_at=timezone.now() - timedelta(days=random.randint(1, 10)),
            status='completed'
        )

        # Create corresponding Result instances
        result = Result.objects.create(
            test=test,
            character_latencies={
                f"{pos}": [random.randint(200, 1000) for _ in range(random.randint(3, 5))]
                for pos in range(3, 10)
            },
            character_accuracies={
                f"{pos}": [random.choice([0.0, 1.0]) for _ in range(random.randint(3, 5))]
                for pos in range(3, 10)
            },
            amount_correct=random.randint(1, 10)
        )
        print(f"Created Test for age group {min_age}-{max_age}: Test ID {test.id}, Result ID {result.id}")
